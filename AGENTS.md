...
| Term | Meaning |
|------|---------|
| **platform** | The entire IAM service. **Platform admin** controls everything on the platform. |
| **tenant** / **organization** | Same thing. In code and UI always **tenant**. I may say org or tenant interchangeably in chat. |
...
| **app user** | Belongs to an app only. No access to tenant or platform resources; uses the platform only to sign in to apps registered on it. |
