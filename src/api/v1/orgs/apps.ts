  ...
  async (c) => {
    ...
    try {
      ...
      const data = apps.map((app) => ({
        ...app,
        memberCount: app._count.memberships,
        _count: undefined,
      }));
      ...
      return c.json(
        ErrorResponseBuilder.database("Failed to fetch apps", dbError.code),
        500
      );
    }
  }
...
orgApps.post(
  ...
  validator("json", (value, c) => {
    ...
    if (!parsed.success) {
      ...
      return c.json(
        ErrorResponseBuilder.validation("Invalid app data", issues),
        400
      );
    }
    return parsed.data;
      ...
      if (dbError.code === "DB_UNIQUE_CONSTRAINT") {
        return c.json(
          ErrorResponseBuilder.conflict(
            "App with this slug or name already exists in this organization"
          ),
          409
        );
      }
      return c.json(
        ErrorResponseBuilder.database("Failed to register app", dbError.code),
        500
      );
    }
  }
    ...
    try {
      ...
      return c.json({
        ...
        data: {
          ...appData,
          memberCount: app._count.memberships,
          apiKeyCount: app._count.userApiKeys,
          _count: undefined,
        },
        requestId,
      });
      ...
      return c.json(
        ErrorResponseBuilder.system("Failed to fetch app details", "SYSTEM_ERROR"),
        500
      );
    }
  }
...
orgApps.put(
  ...
  async (c) => {
    ...
    try {
      ...
      if (dbError.code === "DB_UNIQUE_CONSTRAINT") {
        return c.json(
          ErrorResponseBuilder.conflict("App with this slug already exists"),
          409
        );
      }
      return c.json(
        ErrorResponseBuilder.database("Failed to update app", dbError.code),
        500
      );
    }
  }
...
orgApps.patch(
  ...
  validator("json", (value, c) => {
    ...
    if (!parsed.success) {
      ...
      return c.json(
        ErrorResponseBuilder.validation("Invalid status", issues),
        400
      );
    }
    return parsed.data;
  ...
  async (c) => {
    ...
    try {
      ...
      return c.json(
        ErrorResponseBuilder.system("Failed to update app status", "SYSTEM_ERROR"),
        500
      );
    }
  }
  ...
  async (c) => {
    ...
    try {
      ...
      Logger.info("App deleted", {
        ...
        memberCount: existingApp._count.memberships,
        requestId,
      });
      ...
      return c.json(
        ErrorResponseBuilder.system("Failed to delete app", "SYSTEM_ERROR"),
        500
      );
    }
  }
 ...
 */
orgApps.post(
  "/:orgId/apps/:appId/regenerate-secret",
  verifyAccessTokenMiddleware,
  requireOrgAccess(),
  requireScope("org:apps:write"),
  async (c) => {
    ...
    try {
      ...
      return c.json({
        success: true,
        message:
          "API secret regenerated successfully. Please save this secret - it won't be shown again.",
        data: {
          apiSecret: newSecret,
        },
        requestId,
      ...
      return c.json(
        ErrorResponseBuilder.system("Failed to regenerate API secret", "SYSTEM_ERROR"),
        500
      );
    }
  }
